...
function buildHandler(req, res, body) {
  ...
  // Force tool_choice: any when tools are available - non-Anthropic
  // upstreams in the pool (DeepSeek, Qwen, GLM) routinely close their
  // turn with end_turn after a text-only "I'll do X" preamble even
  // when the prompt is clearly imperative. tool_choice:any is the API
  // hard switch: the model MUST emit a tool_use block.
  //
  // Only applied when:
  //   - request actually has tools (skip title-gen / summarization)
  //   - caller didn't already set tool_choice (don't override explicit
  //     'auto' / 'none' / specific tool requests from Claude Code)
  if (
    parsedBody &&
    ...
    parsedBody.tool_choice = { type: "any" };
    console.log(
      `[footer-proxy] forced tool_choice=any (tools=${parsedBody.tools.length})`
    );
    body = Buffer.from(JSON.stringify(parsedBody), "utf8");
  }
