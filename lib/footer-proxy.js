...
function buildHandler(req, res, body) {
  ...
  // Inject a short system-level nudge for upstream models that have a
  // weaker tool-following habit on long contexts (DeepSeek, some Qwen
  // flavours). Claude Code's local rules and CLAUDE.md are NOT propagated
  // into the API request body - they're TUI-local. So this is the only
  // place we can talk to the upstream model directly.
  //
  // Only injected when the request has actual tools available (no point
  // on title-gen / summarization requests).
  if (parsedBody && Array.isArray(parsedBody.tools) && parsedBody.tools.length > 0) {
    const nudge = (
      "TOOL USAGE: when tools are available and the next step requires one " +
      "(Bash, Read, Edit, Write, Grep, Glob, etc.), call the tool. " +
      "Do not narrate intent without acting - i.e. do NOT write \"I'll first " +
      "check the state, then configure X\" and end the turn without a tool_use " +
      "block. Every \"I'll do X\" must be immediately followed by the tool call " +
      "doing X in the SAME response. Text commentary after tool results is fine; " +
      "text instead of a tool is not."
    );
    ...
    parsedBody.system.unshift({ type: "text", text: nudge });
