...
// Footer injection — opt-in. Footer полезен в интерактивной chat-сессии
// Михаила локально (видно какая модель ответила), но в агентском/сервисном
// режиме мешает: добавляет блок с index 99 в stream, claude CLI его не
// ожидает и теряет реальный content. Default = off. Включить локально:
// FOOTER_PROXY_FOOTER=1.
const FOOTER_ENABLED = process.env.FOOTER_PROXY_FOOTER === "1";
...
// Глобальная версия — удаляет ВСЕ вхождения, не только первое.
const CHAT_TEMPLATE_BOUNDARY_RE_G =
  /<[｜|](?!tool[_▁])(?:im_(?:start|end)|endoftext|begin[_▁]of[_▁]sentence|end[_▁]of[_▁]sentence)[｜|]>/g;

// Раньше эта функция обрезала строку на первом chat-template токене.
// Это ломает reasoning-модели (Qwen3.6, DeepSeek-V3), которые часто
// используют <|im_end|> и подобные как разделитель thinking/answer:
// после токена идёт настоящий ответ, и обрезка его теряла.
//
// Текущее поведение: удаляем только сами токены (replace на ""), оставляя
// остальной текст. Если после токена идёт next-turn галлюцинация (была
// проблемой для legacy моделей), пусть клиент сам её фильтрует — потеря
// легитимного контента хуже.
function truncateAtChatTemplateBoundary(s) {
  if (typeof s !== "string") return s;
  return s.replace(CHAT_TEMPLATE_BOUNDARY_RE_G, "");
}
...
function buildHandler(req, res, body) {
  ...
  // tool_choice hard-switch was attempted here ({"type":"any"}, the
  // Anthropic-shape "any tool must be called" directive), but ccr does
  // NOT translate it into the OpenAI-shape "required" literal when
  // routing to OpenAI-shape upstreams. DeepSeek and friends bounce the
  // Anthropic-shape value with 400 Bad Request:
  //   "Input should be 'auto', 'required' or 'none'"
  // Reverted - we'll come back to this when we can either fix the ccr
  // transformer or detect the upstream shape inside footer-proxy.
  //
  // For now, the architectural reshape above (system-reminder -> system[])
  // is the active mechanism for steering upstream behavior.

  // DEBUG dump goes AFTER all body modifications so the dumped JSON
  // matches what's actually sent to ccr.
  let debugId = null;
  ...
  delete headers["host"];
  // Strip volatile per-session/per-request headers before forwarding: upstream
  // gateways may include them in cache keys, which defeats prefix caching for
  // long contexts (every request re-processes the full prompt).
...
function handleUpstreamResponse(upstreamRes, res) {
  ...
  if (status === 429 || (status >= 500 && status < 600)) {
    // last-decision из памяти router.js (:3458 HTTP-IPC, без файла); fire-and-forget.
    fetch("http://127.0.0.1:3458/last-decision", { signal: AbortSignal.timeout(2000) })
      .then((r) => (r.ok ? r.json() : null))
      .then((d) => { if (d && d.modelId) markQuarantined(d.modelId, META.quarantine_minutes || 5); })
      .catch(() => {});
  }

  const decision = null; // футер убран; decision-сигнал не нужен
  const footer = FOOTER_ENABLED ? formatFooter(decision) : "";

  // Direct pipe только при error-статусе или неизвестном content-type.
  // ВАЖНО: обработка (фильтрация unsupported content blocks, dropping
  // mis-typed deltas, tool-template rescue) выполняется НЕЗАВИСИМО от
  // footer - они нужны для корректности агентского pipeline даже когда
  // footer выключен. Раньше тут было `|| !footer` - это ломало
  // обработку при FOOTER_PROXY_FOOTER=off (default), и thinking-блоки
  // от локальных моделей доходили до claude CLI с ошибкой
  // "Content block is not a text block".
  if (status >= 400 || (!isStream && !isJson)) {
  ...
  if (isStream) {
    ...
    // Track the type of every open block by index. Some upstreams
    // (Qwen3-32B observed 2026-05-04) close a tool_use block by emitting
    // a stray text_delta event for the same index right before
    // content_block_stop. That delta has the wrong type (text_delta in
    // a tool_use block) and Claude Code rejects the message with
    // "Content block is not a text block". We drop those mis-typed
    // deltas before forwarding.
    const blockTypeByIndex = new Map();

    // Re-indexing: claude CLI игнорирует блоки с index != 0..N (плотная
    // нумерация от 0). Если мы дропаем какие-то блоки (thinking и пр.) -
    // оставшиеся надо переименовать в плотную нумерацию.
    const indexMap = new Map(); // orig_idx -> new_idx (только для forward'ыми блоков)
    let nextNewIdx = 0;

    // Fallback на случай если LLM вернула только thinking (или другие
    // unsupported блоки) и ни одного text/tool_use.
    const droppedTextByIndex = new Map(); // idx -> accumulated thinking text
    let sawUsableBlock = false;

    // Переписывает поле "index" в SSE event на новое значение.
    function rewriteEventIndex(event, newIdx) {
      const m = event.match(/^(event: [^\n]+\ndata: )(\{.*\})(\n\n)$/s);
      if (!m) return event;
      try {
        const payload = JSON.parse(m[2]);
        payload.index = newIdx;
        return m[1] + JSON.stringify(payload) + m[3];
      } catch (_) {
        return event;
      }
    }
    ...
    upstreamRes.on("data", (chunk) => {
      ...
      while ((idx = buffer.indexOf("\n\n")) !== -1) {
        ...
        // content_block_start: определяем тип блока. claude CLI в agent
        // режиме поддерживает только text + tool_use в content[]. Любой
        // другой тип (thinking, redacted_thinking, signature и т.п.) - не
        // поддерживается claude CLI и приводит к ошибке "Content block is
        // not a text block". Помечаем такие блоки как DROP и не
        // форвардим ни start, ни последующие delta/stop с этим index.
        if (/"type"\s*:\s*"content_block_start"/.test(event)) {
          const startTypeMatch = event.match(
            /"content_block"\s*:\s*\{\s*"type"\s*:\s*"([a-z_]+)"/
          );
          const m = event.match(/"index"\s*:\s*(\d+)/);
          if (startTypeMatch && m) {
            const blockType = startTypeMatch[1];
            const blockIdx = parseInt(m[1], 10);
            if (blockType === "tool_use") {
              sawToolUseBlock = true;
              sawUsableBlock = true;
              blockTypeByIndex.set(blockIdx, "tool_use");
              const newIdx = nextNewIdx++;
              indexMap.set(blockIdx, newIdx);
              if (newIdx !== blockIdx) {
                out += rewriteEventIndex(event, newIdx);
                continue;
              }
            } else if (blockType === "text") {
              textBlockOpenIdx = blockIdx;
              sawUsableBlock = true;
              blockTypeByIndex.set(blockIdx, "text");
              const newIdx = nextNewIdx++;
              indexMap.set(blockIdx, newIdx);
              if (newIdx !== blockIdx) {
                out += rewriteEventIndex(event, newIdx);
              ...
              // unsupported тип (thinking, signature, image, ...) -
              // дропаем start, накапливаем содержимое для fallback.
              blockTypeByIndex.set(blockIdx, "DROP");
              droppedTextByIndex.set(blockIdx, "");
              console.log(
                `[footer-proxy] dropping unsupported content block ` +
                `(type=${blockType} index=${blockIdx})`
              );
              continue;
            }
          }
        ...
        // DROP-блоки: накапливаем содержимое и не форвардим ни delta, ни stop.
        if (/"type"\s*:\s*"content_block_(delta|stop)"/.test(event)) {
          const im = event.match(/"index"\s*:\s*(\d+)/);
          if (im) {
            const blockIdx = parseInt(im[1], 10);
            if (blockTypeByIndex.get(blockIdx) === "DROP") {
              if (/"type"\s*:\s*"content_block_delta"/.test(event)) {
                ...
                if (dataM) {
                  try {
                    ...
                    const piece =
                      ...
                      "";
                    if (piece) {
                      droppedTextByIndex.set(
                        blockIdx,
                        (droppedTextByIndex.get(blockIdx) || "") + piece
                      );
                    }
                  } catch (_) {}
                }
        ...
        // Drop deltas whose type doesn't match the block they target.
        // text_delta in a tool_use block, or input_json_delta in a text
        // block, both crash Claude Code with "Content block is not a
        // text block" / "...not a tool_use block". Some upstreams emit
        // these sporadically (Qwen3-32B has been observed sending a
        // trailing text_delta inside a tool_use block right before
        // content_block_stop).
        // ВАЖНО: эта проверка должна идти ДО re-indexing, иначе re-indexed
        // delta уходит в out минуя фильтр.
        if (/"type"\s*:\s*"content_block_delta"/.test(event)) {
          const im = event.match(/"index"\s*:\s*(\d+)/);
          if (im) {
            const blockIdx = parseInt(im[1], 10);
            const blockType = blockTypeByIndex.get(blockIdx);
            const isTextDelta = /"type"\s*:\s*"text_delta"/.test(event);
            const isJsonDelta = /"type"\s*:\s*"input_json_delta"/.test(event);
            if (
              (blockType === "tool_use" && isTextDelta) ||
              (blockType === "text" && isJsonDelta)
            ) {
              console.log(
                `[footer-proxy] dropped mis-typed delta ` +
                `(${isTextDelta ? "text_delta" : "input_json_delta"} ` +
                `in ${blockType} block index=${blockIdx})`
              );
              continue; // don't forward, don't add to out
            }
          }
        }

        // Re-indexing для delta/stop: если блок был forward'нут с другим
        // индексом - переписываем index в этом событии тоже.
        if (/"type"\s*:\s*"content_block_(delta|stop)"/.test(event)) {
          const im = event.match(/"index"\s*:\s*(\d+)/);
          if (im) {
            const blockIdx = parseInt(im[1], 10);
            const newIdx = indexMap.get(blockIdx);
            if (newIdx !== undefined && newIdx !== blockIdx) {
              out += rewriteEventIndex(event, newIdx);
              continue;
            }
          }
        ...
        // Forget the type once the block closes.
        if (/"type"\s*:\s*"content_block_stop"/.test(event)) {
          const m = event.match(/"index"\s*:\s*(\d+)/);
          if (m) blockTypeByIndex.delete(parseInt(m[1], 10));
        }
          ...
          if (textDeltaMatch) {
            ...
            if (dataM) {
              try {
                ...
                // Reasoning-модели (Qwen3.6, DeepSeek-V3) легитимно вставляют
                // эти токены как разделитель между thinking и финальным
                // ответом - после токена идёт настоящий контент. Раньше тут
                // был переход в "drop_until_message_delta" который дропал
                // весь оставшийся поток, из-за этого ответы reasoning-моделей
                // терялись. Теперь только очищаем текущую delta и едем
                // дальше как обычно.
                ...
                if (truncated !== text) {
                  ...
                  continue; // эта delta заменена, остальной поток не трогаем
                }
              } catch (_) {}
            }
          }
          // Fallback: если ни одного text/tool_use блока не было forward'нуто,
          // а накопленный thinking есть — синтезируем text-блок перед
          // message_delta. Иначе claude CLI получит пустой content и тихо
          // не выдаст ответа.
          if (
            !sawUsableBlock &&
            ...
            const parts = [];
            for (const t of droppedTextByIndex.values()) {
              if (t) parts.push(t);
            }
            const combined = parts.join("\n\n").trim();
            if (combined) {
              const synthIdx = nextNewIdx++;
              const startEv = {
                type: "content_block_start",
                index: synthIdx,
                content_block: { type: "text", text: "" },
              ...
              const stopEv = { type: "content_block_stop", index: synthIdx };
              ...
              sawUsableBlock = true;
              console.log(
                `[footer-proxy] no usable content block — synthesized text fallback ` +
                `from ${parts.length} dropped block(s), len=${combined.length}`
              );
            }
          }
          // Inject footer before message_delta (only for normal chat replies
          // и только если footer непустой - иначе мы вставим пустой text-блок
          // в конце, который собьёт claude CLI с толку).
          if (
            !footerInjected &&
            !sawToolUseBlock &&
            footer &&
            ...
            const footerIdx = nextNewIdx++;
            out += makeFooterSSE(footerIdx, footer);
            footerInjected = true;
          }
          out += event;
          continue;
        ...
        if (mode === "drop_until_message_delta") {
          ...
          if (/"type"\s*:\s*"message_delta"/.test(event)) {
            ...
            if (!footerInjected && !sawToolUseBlock && footer) {
              const footerIdx = nextNewIdx++;
              out += makeFooterSSE(footerIdx, footer);
              footerInjected = true;
            }
  ...
  upstreamRes.on("end", () => {
    try {
      ...
      if (Array.isArray(json.content)) {
        // Rescue chat-template tool calls в text-блоках.
        // Параллельно собираем text/tool_use + накапливаем thinking-text
        // на случай если text не пришёл (тогда конвертируем thinking в
        // text, иначе claude CLI получает пустой content и не ответит).
        const newContent = [];
        let synthesized = 0;
        const thinkingFallbackParts = [];
        for (const c of json.content) {
          if (c.type === "text" && hasToolTemplateMarker(c.text || "")) {
            ...
            // Накапливаем для fallback - используется только если text/tool_use
            // не пришли вообще.
            thinkingFallbackParts.push(c.thinking);
          } else {
            console.log(
              `[footer-proxy] dropping unsupported content block ` +
              `(type=${c.type}) from non-stream response`
            );
          }
        }
        // Если в content нет ни text ни tool_use - превращаем thinking в text
        // как fallback. Иначе claude CLI получит пустой content и упадёт.
        const hasUsable = newContent.some(
          (c) => c.type === "text" || c.type === "tool_use"
        );
        if (!hasUsable && thinkingFallbackParts.length > 0) {
          console.log(
            `[footer-proxy] no text/tool_use in response - converting ` +
            `${thinkingFallbackParts.length} thinking block(s) to text fallback`
          );
          newContent.push({
            type: "text",
            text: thinkingFallbackParts.join("\n\n"),
          });
        }
